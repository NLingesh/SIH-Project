...
export function Workspace() {
  ...
  return <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
    ...
    {tab === 'evidence' && <Panel title="Evidence"><DataTable headers={['ID', 'SIGNAL', 'FEATURE', 'SCORE', 'CONFIDENCE', 'PROVENANCE']} rows={vm.evidence.map(item => [item.evidence_id, item.signal_type, item.feature, `${item.score}`, `${item.confidence}%`, item.is_synthetic ? 'Synthetic' : item.artifact_name])} empty="No evidence returned for this case." /></Panel>}
    {tab === 'entities' && <Panel title="Identity candidates & aliases"><DataTable headers={['ENTITY', 'TYPE', 'CONFIDENCE', 'ALIASES', 'SOURCE']} rows={vm.entities.map(item => [item.label, item.type, `${item.confidence}%`, item.aliases.join(', ') || '—', item.is_synthetic ? 'Synthetic' : 'Backend'])} empty="No entities returned for this case." /></Panel>}
    {tab === 'signals' && <Panel title="Attribution signal breakdown"><DataTable headers={['SIGNAL', 'SCORE', 'WEIGHT', 'CONTRIBUTION', 'EVIDENCE']} rows={signalRows.map(item => [item.signal, `${item.score}`, `${Math.round(item.weight * 100)}%`, item.contribution.toFixed(1), vm.evidence.filter(e => e.signal_type === item.signal || (item.signal === 'technical' && e.signal_type === 'technical_fingerprint')).length])} empty="No signal results returned." /></Panel>}
    {tab === 'graph' && <Panel title="Identity linkage graph"><DataTable headers={['SOURCE', 'RELATIONSHIP', 'TARGET', 'CONFIDENCE', 'EVIDENCE']} rows={vm.graph.edges.map(edge => [edge.source, edge.type, edge.target, `${edge.confidence}%`, edge.evidence_ids.join(', ') || '—'])} empty="No graph relationships returned." /></Panel>}
